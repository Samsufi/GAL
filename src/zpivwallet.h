...
#ifndef GALR_ZGALWALLET_H
#define GALR_ZGALWALLET_H
...
public:
    CzGALWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZGAL(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZGAL(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //GALR_ZGALWALLET_H
