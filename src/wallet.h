...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 GAL at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zGAL send
enum ZerocoinSpendStatus {
    ZGAL_SPEND_OKAY = 0,                            // No error
    ZGAL_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZGAL_WALLET_LOCKED = 2,                         // Wallet was locked
    ZGAL_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZGAL_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZGAL_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZGAL_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZGAL_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZGAL_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZGAL_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZGAL_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZGAL_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZGAL_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZGAL_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZGAL_SPENT_USED_ZGAL = 14,                      // Coin has already been spend
    ZGAL_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZGAL_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZGALOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzGALWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzGALTracker> zpivTracker;
    ...
    void setZWallet(CzGALWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzGALTracker>(new CzGALTracker(strWalletFile));
    }

    CzGALWallet* getZWallet() { return zwalletMain; }
    ...
    /** zGAL reset */
    boost::signals2::signal<void()> NotifyzGALReset;
