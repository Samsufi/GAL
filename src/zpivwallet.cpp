...
CzGALWallet::CzGALWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGALSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGALSeed_deprecated()) {
                    LogPrintf("%s: Updated zGAL seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzGALWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGALCount(nCountLastUsed);
    else if (!walletdb.ReadZGALCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGALWallet::Lock()
{
    seedMaster = 0;
}

void CzGALWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGALWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGAL(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGALWallet::LoadMintPoolFromDB()
{
...
void CzGALWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGALWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGALWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzGALWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGAL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzGALWallet::SeedToZGAL(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGALWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGALWallet::GenerateDeterministicZGAL(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGALWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGALWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
