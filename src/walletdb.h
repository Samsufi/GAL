...
public:
    ...
    bool WriteZGALSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGALSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGALSeed_deprecated(uint256& seed);
    bool EraseZGALSeed();
    bool EraseZGALSeed_deprecated();

    bool WriteZGALCount(const uint32_t& nCount);
    bool ReadZGALCount(uint32_t& nCount);
